...
class BravePlaylistApiTest : public ExtensionApiTest {
 public:
  BravePlaylistApiTest() {
  ...
  void runTest(const std::string& js) {
    ...
    const Extension* extension =
        LoadExtension(extension_dir_.AppendASCII("bravePlaylist"));
...
IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest, HasAccess) {
  runTest("testHasAccess()");
}

IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest, CreatePlaylistNoCrash) {
  runTest("testCreatePlaylistNoCrash()");
}

IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest, CreatePlaylistValid) {
  runTest(
...
IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest,
...
IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest, ThumbnailFailedButVideoOK) {
  runTest(
...
IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest, ThumbnailOKButVideoFailed) {
  runTest("testCreatePlaylist('" + valid_thumbnail_url() + "', '" +
          invalid_video_url() +
          "', '', ['added','thumbnail_ready','play_ready_partial','aborted'])");
}

IN_PROC_BROWSER_TEST_F(BravePlaylistApiTest, ThumbnailFailedAndVideoFailed) {
  runTest(
      "testCreatePlaylist('" + invalid_thumbnail_url() + "', '" +
      invalid_video_url() +
