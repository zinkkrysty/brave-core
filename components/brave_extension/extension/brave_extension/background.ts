...
function makeMediaItems (item: chrome.bravePlaylists.YTDMediaItem) {
  var mediaFiles: chrome.bravePlaylists.CreateParamsMediaItem[]
  if (typeof item.url === 'string') {
    ...
    mediaFiles = urls.map(
      segment => { return { url: segment, title: item.file}})
  }
  return mediaFiles
}
...
chrome.bravePlaylists.isInitialized((init) => {
  ...
  chrome.bravePlaylists.onInitialized.addListener(() => {
    ...
    chrome.bravePlaylists.onDownloadRequested.addListener((url) => {
      ...
      var videoMediaFiles: chrome.bravePlaylists.CreateParamsMediaItem[] = makeMediaItems(ytdItems[0])
      var audioMediaFiles: chrome.bravePlaylists.CreateParamsMediaItem[] = []
