...
const tipActionCountClass = 'YoutubeTip-actionCount'
...
const createBraveTipAction = (elem: Element, getMetaData: (elem: Element) => RewardsTip.MediaMetaData | null) => {
  ...
  braveTipAction.className = 'YoutubeTip-action js-tooltip ' + actionTipClass + hoverClasses
  braveTipAction.style.display = 'inline-block'
  braveTipAction.style.minWidth = '40px'
  braveTipAction.setAttribute('aria-label', getMessage('youtubeTipsHoverText'))

  // Create the tip button
  const braveTipButton = document.createElement('button')
  braveTipButton.className = 'YoutubeTip-actionButton u-textUserColorHover js-actionButton'
  ...
  braveTipButton.style.padding = '6px 0'
  braveTipButton.style.position = 'relative'
  braveTipButton.type = 'button'
  braveTipButton.onclick = function (event) {
    console.log("CLICKED");
    const youtubeMetaData = getMetaData(elem)
    if (youtubeMetaData) {
      console.log("HAS METADATA");
      const msg = { type: 'tipInlineMedia', mediaMetaData: youtubeMetaData }
  ...
  braveTipActionCountPresentation.className = 'YoutubeTip-actionCountForPresentation'
  braveTipActionCountPresentation.textContent = getMessage('youtubeTipsIconLabel')
  ...
  style.innerHTML = '.YoutubeTip-actionButton :hover { color: #FB542B }'
  shadowRoot.appendChild(style)

  return braveTipAction
}

const getVideoDescriptionMenuMetaData = (elem: Element): RewardsTip.MediaMetaData | null => {
  console.log("GET METADATA");
  const ownerInfo = document.querySelectorAll('yt-formatted-string#owner-name.style-scope.ytd-video-owner-renderer')
  if (ownerInfo) {
    console.log("OWNER INFO");
    const ownerNameLink = ownerInfo[0].getElementsByClassName('yt-simple-endpoint')
    if(ownerNameLink.length) {
      console.log("OWNER INFO CHILDREN");
      const ownerName = ownerNameLink[0] as HTMLElement
      const userName = ownerName.innerHTML
      return {
        mediaType: 'youtube',
        userName: userName || '',
...
const videoDescriptionMenuInsertFunction = (parent: Element) => {
  const tipElem = createBraveTipAction(parent, getVideoDescriptionMenuMetaData)
  if (tipElem.shadowRoot) {
    ...
    braveTipActionCount.style.paddingTop = '8px'
    braveTipActionCount.style.paddingRight = '8px'
    braveTipActionCount.style.fontSize = '1.3rem'
    braveTipActionCount.style.fontWeight = '500'
    braveTipActionCount.style.letterSpacing = '.007px'
    tipElem.style.marginLeft = '8px'
  }
...
const videoDescriptionMenuSelector = '#top-level-buttons'

const configureYoutubeTipAction = (tipLocationSelector: string, tippingEnabled: boolean,
      insertFunction: (parent: Element) => void) => {
  const tipLocations = document.querySelectorAll(tipLocationSelector)
...
const configureYoutubeTipActions = (tippingEnabled: boolean) => {
  let tipLocationSelectors = [
    videoDescriptionMenuSelector,
  ]

  let insertFunctions = [
    videoDescriptionMenuInsertFunction,
  ]

  for (let i in tipLocationSelectors) {
    configureYoutubeTipAction(tipLocationSelectors[i], tippingEnabled, insertFunctions[i])
  }
}
...
const configureBraveTipAction = () => {
  ...
  chrome.runtime.sendMessage('rewardsEnabled', function (rewards) {
    ...
    chrome.runtime.sendMessage(msg, function (inlineTip) {
      ...
      configureYoutubeTipActions(tippingEnabled)
    })
  })
