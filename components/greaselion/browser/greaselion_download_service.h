...
class GreaselionRule {
 public:
  ...
  class Observer : public base::CheckedObserver {
   public:
    virtual void OnRuleReady(GreaselionRule* rule) = 0;
  };
...
class GreaselionDownloadService : public LocalDataFilesObserver,
 ...
 public:
  ...
  void OnRuleReady(GreaselionRule* rule) override;

  // implementation of our own observers
  class Observer : public base::CheckedObserver {
   public:
    virtual void OnAllScriptsLoaded(
        GreaselionDownloadService* download_service) = 0;
  };
  void AddObserver(Observer* observer) { observers_.AddObserver(observer); }
  void RemoveObserver(Observer* observer) {
    observers_.RemoveObserver(observer);
