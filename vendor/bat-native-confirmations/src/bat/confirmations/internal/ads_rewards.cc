...
void AdsRewards::Update(
  ...
  if (!wallet_info_.IsValid()) {
    BLOG(0, "Failed to fetch ads rewards due to invalid wallet");
    return;
  }

  BLOG(1, "Fetch ads rewards");
  GetPaymentBalance();
}
...
void AdsRewards::OnGetPaymentBalance(
  ...
  if (response_status_code != net::HTTP_OK) {
    BLOG(0, "Failed to get payment balance");
    OnAdsRewards(FAILED);
    return;
  }
...
void AdsRewards::OnGetAdGrants(
  ...
  if (response_status_code != net::HTTP_OK) {
    BLOG(0, "Failed to get ad grants");
    OnAdsRewards(FAILED);
    return;
  }
...
void AdsRewards::OnAdsRewards(const Result result) {
  if (result != SUCCESS) {
    BLOG(0, "Failed to retrieve ads rewards");
