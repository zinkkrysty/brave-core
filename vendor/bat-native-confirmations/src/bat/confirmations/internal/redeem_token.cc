...
void RedeemToken::CreateConfirmation(
  ...
  BLOG(1, "POST /v1/confirmation/{confirmation_id}/{credential}");
...
void RedeemToken::FetchPaymentToken(
  ...
  BLOG(1, "GET /v1/confirmation/{confirmation_id}/paymentToken");
...
void RedeemToken::OnFetchPaymentToken(
  ...
  if (response_status_code == net::HTTP_NOT_FOUND) {
    ...
    if (!Verify(confirmation)) {
      BLOG(0, "Failed to verify confirmation");
  ...
  if (response_status_code != net::HTTP_OK) {
    BLOG(0, "Failed to fetch payment token");
  ...
  if (!payment_token_value) {
    BLOG(0, "Response missing paymentToken");
    OnRedeem(FAILED, confirmation, true);
    return;
  }
  ...
  if (!payment_token_value->GetAsDictionary(&payment_token_dictionary)) {
    BLOG(0, "Response missing paymentToken dictionary");
    OnRedeem(FAILED, confirmation, false);
  ...
  if (!public_key_value) {
    BLOG(0, "Response missing publicKey in paymentToken dictionary");
    OnRedeem(FAILED, confirmation, true);
    return;
  }
  ...
  if (!confirmations_->IsValidPublicKeyForCatalogIssuers(public_key_base64)) {
    BLOG(0, "Response public_key: " << public_key_base64 << " was not found "
  ...
  if (!batch_proof_value) {
    BLOG(0, "Response missing batchProof in paymentToken dictionary");
    OnRedeem(FAILED, confirmation, true);
    return;
  }
  ...
  if (!signed_tokens_value) {
    BLOG(0, "Response missing signedTokens in paymentToken dictionary");
    OnRedeem(FAILED, confirmation, true);
    return;
  }
  ...
  if (unblinded_payment_tokens.size() != 1) {
    BLOG(0, "Failed to verify and unblind payment tokens");

    BLOG(0, "  Batch proof: " << batch_proof_base64);

    BLOG(0, "  Payment tokens (" << payment_tokens.size() << "):");
    auto payment_token_base64 = confirmation.payment_token.encode_base64();
    BLOG(0, "    " << payment_token_base64);

    BLOG(0, "  Blinded payment tokens (" << blinded_payment_tokens.size()
        << "):");
    auto blinded_payment_token_base64 =
        confirmation.blinded_payment_token.encode_base64();
    BLOG(0, "    " << blinded_payment_token_base64);
    ...
    BLOG(0, "  Public key: " << public_key_base64);
  ...
  if (unblinded_payment_tokens_->TokenExists(unblinded_payment_token_info)) {
    BLOG(0, "Duplicate unblinded payment token");
    OnRedeem(FAILED, confirmation, false);
    return;
  }
...
void RedeemToken::OnRedeem(
  ...
  if (result != SUCCESS) {
    BLOG(0, "Failed to redeem " << confirmation.id
    ...
    BLOG(1, "Successfully redeemed " << confirmation.id
        << " confirmation id with " << confirmation.creative_instance_id
        << " creative instance id for " << std::string(confirmation.type));
  }
}
